        ...
        <>
          <section className="grid gap-6 lg:grid-cols-3">
            {topThree.map((entry, index) => (
              ...
              >
                <Card.Content className="space-y-4 p-6 text-black">
                  <div className="flex items-center justify-between">
                    <Badge tone="dark">#{entry.rank}</Badge>
                    <Trophy className="size-6" />
                  </div>
                  <div>
                    <div className="font-head text-4xl uppercase">{entry.username}</div>
                  ...
                  <Table.Body>
                    {(rest.length > 0 ? rest : data).map((entry) => (
                      <Table.Row
                        ...
                        className={entry.isCurrentUser ? "bg-[#ffe45e] text-black" : undefined}
                      >
