...
interface GameSessionRepository : JpaRepository<GameSession, String> {
    ...
    fun findByUserIdOrderByStartedAtDesc(
        userId: String,
        pageable: Pageable,
    ): Page<GameSession>
    ...
    fun findTopByUserIdAndStatusOrderByScoreDesc(
        userId: String,
        status: GameStatus,
    ): GameSession?
    ...
    fun findTop10ByCategoryIdAndStatusOrderByScoreDesc(
        categoryId: String,
        status: GameStatus,
    ): List<GameSession>
    ...
    fun findByStatusAndStartedAtAfter(
        status: GameStatus,
        since: LocalDateTime,
    ): List<GameSession>

    @Query(
        "SELECT g FROM GameSession g WHERE g.categoryId = :categoryId AND g.status = :status AND g.startedAt >= :since ORDER BY g.score DESC LIMIT 10",
    )
